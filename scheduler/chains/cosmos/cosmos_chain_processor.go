...
func (ccp *CosmosChainProcessor) queryCycle(ctx context.Context, persistence *queryCyclePersistence) error {
	...
	persistence.latestHeight = status.SyncInfo.LatestBlockHeight
	ccp.log.Info("-----------LatestBlockHeight------------", zap.Int64("persistence.latestHeight", persistence.latestHeight))
	...
	for i := persistence.latestQueriedBlock + 1; i <= persistence.latestHeight; i++ {
		...
		//		// specy
		//		fmt.Println("-------------HandleTxWithTxSpec---------------")
		//		processor.HandleTxWithTxSpec(ctx, tx, chainID, heightUint64, ccp.chainProvider.RPCClient, txIndex)
		//	}
		//}
