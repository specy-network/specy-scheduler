...
func StartScheduler(ctx context.Context, taskHash string, startTime time.Time, interval time.Duration) {
	startGoroutine(ctx, goroutines, taskHash, startTime, interval)
}

func startGoroutine(ctx context.Context, goroutines map[string]chan struct{}, taskHash string, startTime time.Time, interval time.Duration) {
	...
	go func() {
		...
		for {
			...
			dispatcher.ExecuteTask(ctx, taskHash)
